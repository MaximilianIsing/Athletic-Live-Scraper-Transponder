 ...
 * that appear before the first event whose name matches the given event name.
 * @param {string} url - Link to the meet/event page
 * @param {string} eventName - The specific event name to find (e.g. "Boys 55mH", "Girls 200m")
 * @returns {Promise<number>} The number of events before that event, or -1 if not found
 */
export async function numEventsBefore(url, eventName) {
  const events = await getEventListElements(url);
  const index = events.findIndex((ev) => ev.name === eventName);
